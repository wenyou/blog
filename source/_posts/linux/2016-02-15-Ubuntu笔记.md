...
```
    ...
    更新一下源:sudo apt-get update    (有些软件使用apt-get install安装不上，提示Err http://security.ubuntu.com/ubuntu/ trusty-security/main mysql-server-core-5.5 amd64 5.5.47-0ubuntu0.14.04.1 404  Not Found [IP: 91.189.88.152 80] E: Unable to fetch some archives, maybe run apt-get update or try with --fix-missing? 这类的找不到错误，使用sudo apt-get update更新一下系统即可。)
